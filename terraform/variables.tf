...
variable "cloudflare_api_token" {
  description = "Cloudflare API token with permissions to manage target zone and Authenticated Origin Pulls settings."
  type        = string
  sensitive   = true
}

variable "cloudflare_hostname" {
  description = "Hostname created in Cloudflare and proxied to GCP global load balancer IP."
  type        = string
}
...
variable "gateway_hostname" {
  description = "Hostname bound on external multi-cluster Gateway listener and HTTPRoute."
  type        = string
}

variable "gateway_static_ip_name" {
  description = "Name of reserved global static IP attached to external global load balancer."
  type        = string
  default     = "mcg-external-ip"
}

variable "gateway_dns_authorization_name" {
  description = "Name of Certificate Manager DNS authorization used for gateway hostname."
  type        = string
  default     = "gateway-dns-auth"
}

variable "gateway_certificate_name" {
  description = "Name of Certificate Manager certificate used by external gateway."
  type        = string
  default     = "gateway-certificate"
}

variable "gateway_certificate_map_name" {
  description = "Name of Certificate Manager certificate map attached to external gateway."
  type        = string
  default     = "gateway-cert-map"
}

variable "gateway_certificate_map_entry_name" {
  description = "Name of Certificate Manager certificate map entry for gateway hostname."
  type        = string
  default     = "gateway-cert-entry"
}

variable "machine_type" {
  description = "Machine type used by default node pool in both clusters."
  type        = string
  default     = "e2-standard-2"
}

variable "network_name" {
  description = "Name of shared VPC network used by both clusters."
  type        = string
  default     = "gke-mcg-vpc"
}

variable "node_count" {
  description = "Static node count for default node pool in each cluster."
  type        = number
  default     = 2
}
